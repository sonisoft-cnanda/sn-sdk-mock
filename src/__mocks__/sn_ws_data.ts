...
export class MockRESTResponseV2 {
    ...
    getBody = jest.fn().mockImplementation((): any => {
      return RESTDataStore.getInstance().mockResponseBody;
        
    });
  
    getStatusCode = jest.fn().mockImplementation(() : number => {
        return RESTDataStore.getInstance().mockResponseCode;
    });
  
    getRequest = jest.fn().mockImplementation((): MockRESTMessageV2 => {
        return this._mock_request;
    });

    getHeaders = jest.fn().mockImplementation((): Record<any, any> => {
        return this._headers;
    });

    getHeader = jest.fn().mockImplementation((headerName: string): string => {
        return this._headers[headerName];
    });

    haveError = jest.fn().mockImplementation((): boolean => {
  ...
  export class MockRESTMessageV2 {
    ...
    constructor(name?: string, methodName?: string) {
      RESTDataStore.getInstance().addMockRequest(this);
        ...
        if(name && methodName){
            ...
            if(restMessage){
                debug(JSON.stringify("RESTMessage object found in data store." + restMessage));
                ...
                if(restMessageFn){
                    ...
                    error("RESTMessageFunctionTemplate not found for " + name + "." + methodName);
                }
            }else{
    ...
    getRequestHeader = jest.fn().mockImplementation((headerName?: string): string => {
        if(headerName){
    ...
    getRequestHeaders = jest.fn().mockImplementation((): Record<any, any> => {
        return this._headers;
    });

    getRequestBody = jest.fn().mockImplementation(() => {
        return this._restMessageBody;
    });

    setRequestHeader = jest.fn().mockImplementation((name:string, value:string) => {
    ...
    setMIDServer = jest.fn().mockImplementation((value: string): void =>  {
    ...
    setEccParameter = jest.fn().mockImplementation((param: string, value: string): void =>{
        this.mockEccParams[param] = value;
    });

    setHttpMethod = jest.fn().mockImplementation((method: string): void => {
        this.mockProperties["http_method"] = method;
    });

    setEndpoint = jest.fn().mockImplementation((endpoint: string): void => {
        this._endpoint = endpoint;
    });
  
    setRequestBody = jest.fn().mockImplementation((body: string): void => {
        this._restMessageBody = body;
    });
